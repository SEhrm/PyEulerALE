...
lift_wrt_aoa_wrt_mach = (
    np.vdot(
        solver.compute_forces_wrt_states_inner_product_wrt_states(
            d_states=states_wrt_aoa, d_forces=lift_wrt_forces_adj,
        ), states_wrt_mach,
    ) +
    np.vdot(
        solver.compute_forces_wrt_vertices_inner_product_wrt_states(
            d_vertices=vertices_wrt_aoa, d_forces=lift_wrt_forces_adj,
        ), states_wrt_mach,
    ) +
    solver.compute_odes_wrt_states_inner_product_wrt_mach(
        d_states=states_wrt_aoa, d_odes=lift_wrt_odes_adj,
    ).item() +
    np.vdot(
        solver.compute_odes_wrt_states_inner_product_wrt_states(
            d_states=states_wrt_mach, d_odes=lift_wrt_odes_adj,
        ), states_wrt_aoa,
    ) +
    solver.compute_odes_wrt_vertices_inner_product_wrt_mach(
        d_vertices=vertices_wrt_aoa, d_odes=lift_wrt_odes_adj,
    ).item() +
    np.vdot(
