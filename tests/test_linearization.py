...
class TestJacobi(unittest.TestCase):
    ...
    def test_odes_wrt_states_inner_product_wrt_mach(self) -> None:
        ...
        self.solver.linearize()
        np.testing.assert_allclose(
            self.solver.compute_odes_wrt_states_inner_product_wrt_mach(d_states, d_odes),
    ...
    def test_odes_wrt_states_inner_product_wrt_states(self) -> None:
        ...
        self.solver.linearize()
        np.testing.assert_allclose(
            self.solver.compute_odes_wrt_states_inner_product_wrt_states(d_states, d_odes),
            gradient, atol=1e-6, rtol=1e-5,
        )
    ...
    def test_odes_wrt_states_inner_product_wrt_vertices(self) -> None:
        ...
        self.solver.linearize()
        np.testing.assert_allclose(
            self.solver.compute_odes_wrt_states_inner_product_wrt_vertices(d_states, d_odes),
    ...
    def test_odes_wrt_vertices_inner_product_wrt_mach(self) -> None:
        ...
        d_vertices = random_like(self.solver.vertices)
        d_odes = random_like(self.solver.odes)
        gradient = np.zeros((1,), dtype=complex)
        mach_0 = self.solver.mach_number
        ...
        self.solver.linearize()
        np.testing.assert_allclose(
            self.solver.compute_odes_wrt_vertices_inner_product_wrt_mach(d_vertices, d_odes),
            gradient, atol=1e-6, rtol=1e-5,
        )
    ...
    def test_odes_wrt_vertices_inner_product_wrt_states(self) -> None:
        ...
        self.solver.linearize()
        np.testing.assert_allclose(
            self.solver.compute_odes_wrt_vertices_inner_product_wrt_states(d_vertices, d_odes),
            gradient, atol=1e-6, rtol=1e-5,
        )
    ...
    def test_forces_wrt_states_inner_product_wrt_states(self) -> None:
        ...
        d_states = random_like(self.solver.states)
        d_forces = random_like(self.solver.forces)
        gradient = np.zeros_like(self.solver.states, dtype=complex)
        states_0 = self.solver.states.copy()
        ...
        self.solver.linearize()
        np.testing.assert_allclose(
            self.solver.compute_forces_wrt_states_inner_product_wrt_states(d_states, d_forces),
            gradient, atol=1e-6, rtol=1e-5,
        )
    ...
    def test_forces_wrt_vertices_inner_product_wrt_states(self) -> None:
        ...
        self.solver.linearize()
        np.testing.assert_allclose(
            self.solver.compute_forces_wrt_vertices_inner_product_wrt_states(d_vertices, d_forces),
            gradient, atol=1e-6, rtol=1e-5,
        )
