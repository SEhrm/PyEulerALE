...
\begin{gathered}
∇_{Ma_\infty, 𝓤, 𝓧}
\left.\left\langle δ𝓡, \frac{∂𝓡}{∂𝓤}\,δ𝓤 \right\rangle
\right\vert_{δ𝓡,δ𝓤}
, \quad
∇_{Ma_\infty, 𝓤, 𝓧}
\left.\left\langle δ𝓡, \frac{∂𝓡}{∂𝓧}\,δ𝓧 \right\rangle
\right\vert_{δ𝓡,δ𝓤}
, \quad
∇_{Ma_\infty, 𝓤, 𝓧}
\left.\left\langle δ𝓡, \frac{∂𝓡}{∂𝓥}\,δ𝓥 \right\rangle
\right\vert_{δ𝓡,δ𝓤}
\\
∇_{𝓤, 𝓧}
\left.\left\langle δ𝓕, \frac{∂𝓕}{∂𝓤}\,δ𝓤 \right\rangle
\right\vert_{δ𝓡,δ𝓤}
, \quad
∇_{𝓤, 𝓧}
\left.\left\langle δ𝓕, \frac{∂𝓕}{∂𝓧}\,δ𝓧 \right\rangle
\right\vert_{δ𝓕,δ𝓧}
\text{.}
\end{gathered}
$$
...
At the steady-state, the transfer functions from pitch angle $`α`$ to coefficients of lift and
moment at Laplace variable $`s`$ read

$$
\left.\frac{ℒ(c_\text{l},c_\text{m})}{ℒα}\right\vert_s =
\frac{∂(c_\text{l},c_\text{m})}{∂𝓧}
\frac{\mathrm{d}𝓧}{\mathrm{d}α} +
\frac{∂(c_\text{l},c_\text{m})}{∂𝓕}
\left.\frac{ℒ𝓕}{ℒα}\right\vert_s
\text{,}
$$
...
The gain $`\mathrm{d}𝓧\textfractionsolidus\mathrm{d}α`$ follows from the
rotation
around $`\vec{x}_\text{a}`$ and the gains
$`∂(c_\text{l},c_\text{m})\textfractionsolidus∂𝓕`$ and
$`∂(c_\text{l},c_\text{m})\textfractionsolidus∂𝓧`$ follow from the
definition of the (classical) coefficients of section lift and moment

$$
c_\text{l} := \dfrac{\oint f_z \mathrm{d}s}
{\dfrac{ϱ_\infty}{2} u_\infty^2 c}, \quad
c_\text{m} := \dfrac{\oint f_x z - f_z (x - x_\text{a}) \mathrm{d}s}
{\dfrac{ϱ_\infty}{2} u_\infty^2 c^2}\text{.}
$$
...
of reduced frequency $`(ω c)\textfractionsolidus(2 u_\infty)`$. The results compare well to the
...
$$
c_\text{p} := \dfrac{p - p_\infty}{ϱ_\infty u_\infty^2\textfractionsolidus 2}
$$

and a frequency response $`ℒc_\text{p}\textfractionsolidusℒα`$ at
$`ω = 2 u_\infty \textfractionsolidus c`$.
...
\left| \frac{ℒc_\text{p}}{ℒα} \right| \cdot
\cos\left( ω t + \angle\left( \frac{ℒc_\text{p}}{ℒα} \right)\right)
$$
...
\begin{aligned}
∇_{Ma_\infty}\left(
\left.\frac{ℒc_\text{l}}{ℒα}\right\vert_{s=0}
\right) &=
-2 Ma_\infty^{-1}\cdot\left.\frac{ℒc_\text{l}}{ℒα}\right\vert_{s=0} +
∇_{Ma_\infty}\left\langle
∇_{𝓕}{c_\text{l}},
\frac{∂𝓕}{∂{𝓧}}\frac{\mathrm{d}𝓧}{\mathrm{d}α}
\right\rangle +
∇_{Ma_\infty}\left\langle
∇_{𝓕}{c_\text{l}},
\frac{∂𝓕}{∂{𝓤}}\left.\frac{ℒ𝓤}{ℒα}\right\vert_{s=0}
\right\rangle
\text{,}
\end{aligned}
$$
...
\frac{∂𝓕}{∂{𝓧}}\frac{\mathrm{d}𝓧}{\mathrm{d}α}
\right\rangle =
\left\langle ∇_{𝓤} \left\langle ∇_{𝓕}{c_\text{l}},
\frac{∂𝓕}{∂{𝓧}}\frac{\mathrm{d}𝓧}{\mathrm{d}α}
\right\rangle,
...
$$
\begin{aligned}
∇_{Ma_\infty}\left\langle ∇_{𝓕}{c_\text{l}},
\frac{∂𝓕}{∂{𝓤}}\left.\frac{ℒ𝓤}{ℒα}\right\vert_{s=0}
\right\rangle &=
∇_{Ma_\infty}\left\langle ∇_{𝓕}{c_\text{l}},
\frac{∂𝓕}{∂𝓤} \left(-\frac{∂𝓡}{∂𝓤}\right)^{-1} \frac{∂𝓡}{∂𝓧} \frac{\mathrm{d}𝓧}{\mathrm{d}α}
\right\rangle \\
&=
\left\langle ∇_{𝓤}\left\langle ∇_{𝓕}{c_\text{l}},
\frac{∂𝓕}{∂𝓤} \left.\frac{ℒ𝓤}{ℒα}\right\vert_{s=0}
...
\right\rangle\\
&+
\left\langle ∇_{𝓤}\left\langle
∇_{𝓡}{c_\text{l}},
\frac{∂𝓡}{∂𝓤} \left.\frac{ℒ𝓤}{ℒα}\right\vert_{s=0}
\right\rangle,
\left.\frac{ℒ𝓤}{ℒMa_\infty}\right\vert_{s=0}
\right\rangle+
∇_{Ma_\infty}\left\langle
∇_{𝓡}{c_\text{l}},
\left.\frac{∂𝓡}{∂𝓤}\right\vert_{𝓤} \left.\frac{ℒ𝓤}{ℒα}\right\vert_{s=0}
\right\rangle\\
&+
\left\langle ∇_{𝓤}\left\langle
∇_{𝓡}{c_\text{l}},
\frac{∂𝓡}{∂𝓧} \left.\frac{ℒ𝓧}{ℒα}\right\vert_{s=0}
\right\rangle,
\left.\frac{ℒ𝓤}{ℒMa_\infty}\right\vert_{s=0}
\right\rangle+
∇_{Ma_\infty}\left\langle
∇_{𝓡}{c_\text{l}},
\left.\frac{∂𝓡}{∂𝓧}\right\vert_{𝓤} \left.\frac{ℒ𝓧}{ℒα}\right\vert_{s=0}
\right\rangle\\
\end{aligned}
$$
...
$$
∇_{𝓡}{c_\text{l}} :=
\left(-\frac{∂𝓡}{∂𝓤}\right)^{-\dagger}
\left(\frac{∂𝓕}{∂𝓤}\right)^{\dagger}
∇_{𝓕}{c_\text{l}}\text{.}
$$
